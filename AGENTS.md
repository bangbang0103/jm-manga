...
- 当前版本构建 Android APK、iOS 未签名 IPA，以及 Windows / macOS 桌面 zip（ad-hoc 签名）；不构建 Linux 与 Web 包。
...
- 横屏适配与 Windows/macOS 平台扩展的评估与计划写入 `docs/platform-expansion-plan.md`（实施落地后需更新上述平台政策条目）。
