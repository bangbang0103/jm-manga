...
class _LogsScreenState extends State<LogsScreen> {
  ...
  bool _matchesSearch(LogEntry entry) {
    if (_searchQuery.isEmpty) return true;
    final query = _searchQuery.toLowerCase();
    return entry.message.toLowerCase().contains(query) ||
        (entry.error?.toString().toLowerCase().contains(query) ?? false) ||
        entry.formattedTime.toLowerCase().contains(query) ||
        entry.level.label.toLowerCase().contains(query);
  }
  ...
  void _showEntry(LogEntry entry) {
          ...
          content: SingleChildScrollView(
            child: SelectableText(
              [
                ...
                if (entry.stackTrace != null) entry.stackTrace.toString(),
              ].join('\n\n'),
