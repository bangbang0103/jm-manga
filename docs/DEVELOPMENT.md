...
scripts/
  release.sh                 统一发布构建入口（mobile / desktop / all）
  build-flutter.sh           Flutter 构建与 sha256 校验
  sync-version.sh            将根 VERSION 同步到 Flutter 版本文件
...
```bash
./scripts/release.sh all       # 移动端（Android APK + iOS 未签名 IPA）
./scripts/release.sh mobile    # 同上
